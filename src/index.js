...
SALES_DEMOS['bonbuket-sevastopol'] = {
  ...
  subheadline: 'Современная цветочная мастерская с понятным каталогом, быстрым заказом и акцентом на визуальную подачу.',
  phone: '+7 978 332-43-44',
  telegram: '@BonBuketSevas',
  primary_action_label: 'Заказать букет',
  services: [
  ...
  benefits: [
    { title: 'Живой каталог', description: 'Покупатель сразу видит стиль работ и варианты заказа.' },
    { title: 'Заказ в один клик', description: 'Телефон и Telegram доступны прямо с первого экрана.' },
    { title: 'Подходит для рекламы', description: 'Сайт удобно использовать как посадочную страницу из соцсетей.' }
  ],
...
app.get('/v1/maintenance/rebuild-vard-20260908', async function (req, res) {
  ...
  const assetPaths=[
    'fonts/manrope-400.woff2','fonts/manrope-500.woff2','fonts/manrope-600.woff2','fonts/prata-400.woff2',
    'assets/hero.jpg','assets/store.jpg',
    'assets/assortment-1.jpg','assets/assortment-2.jpg','assets/assortment-3.jpg',
    'assets/assortment-4.jpg','assets/assortment-5.jpg','assets/assortment-6.jpg',
    'assets/gallery-1.jpg','assets/gallery-2.jpg','assets/gallery-3.jpg','assets/gallery-4.jpg','assets/gallery-5.jpg',
    'content.js'
  ];

  const compatibleContentJs="var VARD_API='https://blue-cvard-flowers-cms-apiloud-58a4.lolikingor18.workers.dev';\n(function(){\n  if(!window.fetch||!document.querySelector){return;}\n  function q(s){return document.querySelector(s);}\n  function qa(s){return document.querySelectorAll(s);}\n  fetch(VARD_API+'/content',{cache:'no-store'})\n    .then(function(r){if(!r.ok){throw new Error('CMS '+r.status);}return r.json();})\n    .then(function(d){\n      var el,i,intro,ps,cards,a,img,b,h,p,br,l,g,links,cp,strong;\n      if(d.heroText&&(el=q('.hero p')))el.textContent=d.heroText;\n      if(d.heroImage&&(el=q('.hero')))el.style.backgroundImage='linear-gradient(90deg,#18241ddd 8%,#18241d9c 45%,transparent 75%),url(\"'+d.heroImage+'\")';\n      if(d.storeImage&&(el=q('.portrait')))el.style.backgroundImage='url(\"'+d.storeImage+'\")';\n      intro=q('#about .intro>div:first-child');\n      if(intro){\n        if(d.aboutTitle&&(el=intro.querySelector('h2')))el.innerHTML=String(d.aboutTitle).replace(/\\s+/,'<br>');\n        ps=intro.querySelectorAll('p');\n        if(ps[0]&&d.aboutText)ps[0].textContent=d.aboutText;\n        if(ps[1]&&d.aboutText2)ps[1].textContent=d.aboutText2;\n      }\n      if(Array.isArray(d.items)&&(cards=q('.cards'))){\n        cards.innerHTML='';\n        d.items.forEach(function(x){\n          a=document.createElement('article');a.className='card';\n          img=document.createElement('img');img.src=x.image;img.alt=x.name||'Букет';\n          b=document.createElement('div');b.className='card-body';\n          h=document.createElement('h3');h.textContent=x.name||'';\n          p=document.createElement('span');p.className='price';p.textContent=x.price||'';\n          br=document.createElement('br');\n          l=document.createElement('a');l.href='tel:'+(d.phoneLink||'');l.textContent=x.description||'Уточнить наличие';\n          b.appendChild(h);b.appendChild(p);b.appendChild(br);b.appendChild(l);\n          a.appendChild(img);a.appendChild(b);cards.appendChild(a);\n        });\n      }\n      if(Array.isArray(d.gallery)&&(g=q('.gallery'))){\n        g.innerHTML='';\n        d.gallery.forEach(function(src,idx){\n          img=document.createElement('img');img.src=src;img.alt='Vard Flowers '+(idx+1);g.appendChild(img);\n        });\n      }\n      links=qa('a[href^=\"tel:\"]');\n      for(i=0;i<links.length;i++)links[i].href='tel:'+(d.phoneLink||'');\n      links=qa('a[href^=\"https://t.me/\"]');\n      for(i=0;i<links.length;i++)links[i].href=d.telegram||'#';\n      cp=q('#contacts p');\n      if(cp){\n        cp.innerHTML='';\n        strong=document.createElement('b');strong.textContent=d.phone||'';\n        cp.appendChild(strong);cp.appendChild(document.createElement('br'));\n        cp.appendChild(document.createTextNode(d.address||''));cp.appendChild(document.createElement('br'));\n        cp.appendChild(document.createTextNode(d.hours||''));\n      }\n    })\n    .catch(function(e){if(window.console&&console.warn)console.warn('CMS unavailable',e);});\n})();";
  const compatibilityCss="\n/* VELORA cross-device compatibility patch 2026-09-08 */\nhtml{width:100%;max-width:100%;overflow-x:hidden;-webkit-text-size-adjust:100%;text-size-adjust:100%}\nbody{width:100%;min-width:0;max-width:100%;overflow-x:hidden}\n*,*::before,*::after{box-sizing:border-box}\nimg,video,picture,canvas,svg,iframe{max-width:100%}\n.wrap{width:90%;max-width:1180px;margin-left:auto;margin-right:auto}\n.nav{top:0;left:0;right:0;bottom:auto;background:#f8f5ef;background:#f8f5efe8;-webkit-backdrop-filter:blur(18px);backdrop-filter:blur(18px)}\n.hero{align-items:flex-end}\n.hero h1,.title{font-family:Prata,Georgia,serif;font-size:72px;line-height:.94;font-weight:400}\n.intro .title,.head .title{font-size:54px;line-height:1.03}\n.actions,.contact-actions{flex-wrap:wrap}\n.btn,.ghost{max-width:100%;text-align:center;touch-action:manipulation;-webkit-tap-highlight-color:transparent}\n.hero p,.intro p,.head p,.contact p,.review blockquote,h1,h2,h3{overflow-wrap:break-word}\n@supports (font-size:clamp(1px,2vw,3px)){\n  .hero h1,.title{font-size:clamp(52px,8vw,110px)}\n  .intro .title,.head .title{font-size:clamp(42px,5vw,68px)}\n}\n@media(max-width:760px){\n  .wrap{width:calc(100% - 28px);max-width:none}\n  .hero{min-height:100vh;min-height:100svh;padding-top:100px;padding-bottom:110px}\n  .hero h1{font-size:52px;line-height:.98}\n  .hero p{font-size:16px;line-height:1.5}\n  .actions{display:grid;grid-template-columns:1fr;gap:8px}\n  .actions .btn,.actions .ghost{width:100%;min-height:48px}\n  .hero-meta{width:100%;max-width:100%;gap:0}\n  .hero-meta div{min-width:0}\n  .intro,.contact-grid{width:100%;grid-template-columns:minmax(0,1fr)}\n  .cards,.reviews{grid-template-columns:minmax(0,1fr)}\n  .gallery{width:100%;max-width:100%}\n  .steps{grid-template-columns:minmax(0,1fr) minmax(0,1fr)}\n  .reason{min-width:210px}\n  .contact .title{font-size:42px}\n  .map{width:100%;min-height:320px}\n  .mobilebar{padding-bottom:10px;padding-bottom:max(10px,env(safe-area-inset-bottom))}\n  .mobilebar .btn{min-width:0;padding-left:10px;padding-right:10px}\n}\n@media(max-width:380px){\n  .wrap{width:calc(100% - 20px)}\n  .hero h1{font-size:44px}\n  .title,.intro .title,.head .title{font-size:38px}\n  .steps{grid-template-columns:1fr}\n  .card img{height:320px}\n  .gallery{grid-template-rows:300px 190px 190px}\n  .contact .title{font-size:36px}\n}\n";

  async function fetchBinary(relativePath) {
    const url=sourceRoot+'/'+relativePath+'?velora-clone=20260908';
    const r=await axios.get(url,{
      timeout:30000,responseType:'arraybuffer',maxRedirects:5,validateStatus:()=>true,
      headers:{'User-Agent':'Velora-Cross-Device-Migration/1.0'}
    });
    if(r.status<200||r.status>=400) throw new Error('Asset fetch failed '+relativePath+' -> '+r.status);
    return Buffer.from(r.data);
  }

  async function deviceQa(url) {
    const profiles=[
      {name:'windows',viewport:{width:1366,height:768},isMobile:false,hasTouch:false,userAgent:'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/140.0.0.0 Safari/537.36'},
      {name:'android',viewport:{width:360,height:800},isMobile:true,hasTouch:true,userAgent:'Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Mobile Safari/537.36'},
      {name:'iphone',viewport:{width:390,height:844},isMobile:true,hasTouch:true,userAgent:'Mozilla/5.0 (iPhone; CPU iPhone OS 17_0 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/17.0 Mobile/15E148 Safari/604.1'}
    ];
    browser=await chromium.launch({
    ...
    for(const profile of profiles){
      const context=await browser.newContext({
        viewport:profile.viewport,isMobile:profile.isMobile,hasTouch:profile.hasTouch,userAgent:profile.userAgent
      });
      const page=await context.newPage();
      const pageErrors=[];
      page.on('pageerror',function(err){pageErrors.push(String(err.message||err));});
      const response=await page.goto(url+'/?velora-qa='+Date.now(),{waitUntil:'networkidle',timeout:45000});
      await page.waitForTimeout(600);
      const metrics=await page.evaluate(function(){
        var h1=document.querySelector('h1');
        var rect=h1?h1.getBoundingClientRect():null;
        return {
          innerWidth:window.innerWidth,
          scrollWidth:document.documentElement.scrollWidth,
          bodyTextLength:(document.body&&document.body.innerText||'').trim().length,
          h1Visible:!!(rect&&rect.width>0&&rect.height>0),
          viewportMeta:!!document.querySelector('meta[name="viewport"]')
        };
      });
      const httpStatus=response?response.status():0;
      const passed=httpStatus>=200&&httpStatus<400&&metrics.bodyTextLength>500&&metrics.h1Visible&&
        metrics.viewportMeta&&metrics.scrollWidth<=metrics.innerWidth+4;
      results.push({device:profile.name,passed,http_status:httpStatus,width:metrics.innerWidth,
        scroll_width:metrics.scrollWidth,body_text_length:metrics.bodyTextLength,page_errors:pageErrors.slice(0,3)});
      await context.close();
    }
    await browser.close();browser=null;
    return {passed:results.every(function(x){return x.passed;}),results};
  }

  try{
    const sourceResp=await axios.get(sourceRoot+'/?velora-source=20260908',{
      timeout:30000,responseType:'text',transformResponse:[function(data){return data;}],
      maxRedirects:5,validateStatus:()=>true,headers:{'User-Agent':'Velora-Cross-Device-Migration/1.0'}
    });
    const originalHtml=String(sourceResp.data||'');
    if(sourceResp.status<200||sourceResp.status>=400||originalHtml.indexOf('Vard Flowers')<0){
      return res.status(502).json({status:'ERROR',error:'Could not load current VARD HTML',source_status:sourceResp.status});
    }

    tempDir=path.join('/tmp','velora-vard-cross-device-'+Date.now());
    fs.mkdirSync(tempDir,{recursive:true});
    fs.writeFileSync(path.join(tempDir,'index.html'),originalHtml);

    for(const relativePath of assetPaths){
      const bytes=await fetchBinary(relativePath);
      const filePath=path.join(tempDir,relativePath);
      fs.mkdirSync(path.dirname(filePath),{recursive:true});
      fs.writeFileSync(filePath,bytes);
    }

    await ensureCloudflareProject(backupProject);
    const backupUrl=await deployToCloudflarePages(tempDir,backupProject,CLOUDFLARE_ACCOUNT_ID,CLOUDFLARE_API_TOKEN);
    ...
    html=html.replace(/%%2C/g,'%2C');
    html=html.replace('</head>','<style id="velora-cross-device">'+compatibilityCss+'</style></head>');
    html=html.replace(/target=["']_blank["']/g,'target="_blank" rel="noopener"');
    fs.writeFileSync(path.join(tempDir,'index.html'),html);
    fs.writeFileSync(path.join(tempDir,'content.js'),compatibleContentJs);

    await ensureCloudflareProject(testProject);
    const testUrl=await deployToCloudflarePages(tempDir,testProject,CLOUDFLARE_ACCOUNT_ID,CLOUDFLARE_API_TOKEN);
    const qa=await deviceQa(testUrl);
    if(!qa.passed){
      return res.status(200).json({status:'ERROR',error:'CROSS_DEVICE_QA_FAILED',backup_url:backupUrl,test_url:testUrl,qa});
    }

    await ensureCloudflareProject(projectName);
    const productionUrl=await deployToCloudflarePages(tempDir,projectName,CLOUDFLARE_ACCOUNT_ID,CLOUDFLARE_API_TOKEN);
    return res.json({status:'OK',site_result_url:productionUrl,backup_url:backupUrl,test_url:testUrl,generator:GENERATOR_VERSION,qa});
  }catch(error){
    return res.status(500).json({status:'ERROR',error:error.message});
  }finally{
    if(browser){try{await browser.close();}catch(e){}}
